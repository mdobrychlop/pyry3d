...
class CCP4:
    ...
    def read_header_values(self, file, file_size):
        ...
        if v['nsymbt'] < 0 or v['nsymbt'] + MRC_HEADER_LENGTH > file_size:
            raise SyntaxError, ('MRC header value nsymbt (%d) is invalid. Check whether file with density map is provided in a correct format'
                          % v['nsymbt'])
    ...
    def read_simulated_volume_fast(self,  kmax, first_complex):
        ...
        molecule_mass = kmax*(first_complex.volume) 
	...
	counts = where(self.densities>self.threshold)
        ...
        for count in counts[0]:
            ...
            count = count +1
            #print x,y,z, density_value, count
